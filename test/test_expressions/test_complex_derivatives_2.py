...
def test_sin_polynomial_log():
    ...
    evaluate_derivative(
        expr,
        lambda x_val: math.cos(x_val**3 + math.log(x_val)) * (3 * x_val**2 + 1 / x_val),
        VALUES,
    )
...
def test_sinh_cos_polynomial():
    ...
    evaluate_derivative(
        expr,
        lambda x_val: math.cosh(x_val) * math.cos(x_val**2)
        - 2 * x_val * math.sinh(x_val) * math.sin(x_val**2),
        VALUES,
    )
...
def test_division_power_polynomial():
    ...
    evaluate_derivative(
        expr,
        lambda x_val: (
            (2 * x_val) * (x_val + 1) ** 3 - (x_val**2 + 1) * 3 * (x_val + 1) ** 2
        )
        / (x_val + 1) ** 6,
        VALUES,
    )
...
def test_tanh_polynomial_trig():
    ...
    evaluate_derivative(
        expr,
        lambda x_val: 1
        / math.cosh(x_val**2 + math.sin(x_val)) ** 2
        * (2 * x_val + math.cos(x_val)),
        VALUES,
    )
...
def test_log_polynomial_sum():
    ...
    evaluate_derivative(
        expr, lambda x_val: (2 * x_val + 1) / (x_val**2 + x_val + 1), VALUES
    )
...
def test_cos_polynomial_trig_product():
    ...
    evaluate_derivative(
        expr,
        lambda x_val: -math.sin(x_val**3 * math.sin(x_val))
        * (3 * x_val**2 * math.sin(x_val) + x_val**3 * math.cos(x_val)),
        VALUES,
    )
...
def test_power_variable_exponent():
    ...
    evaluate_derivative(
        expr,
        lambda x_val: (x_val + 1) ** x_val
        * (math.log(x_val + 1) + x_val / (x_val + 1)),
        VALUES,
    )
...
def test_trig_hyper_log():
    ...
    evaluate_derivative(
        expr,
        lambda x_val: math.cos(math.tanh(x_val)) * (1 / math.cosh(x_val) ** 2)
        + math.tanh(x_val),
        VALUES,
    )
...
def test_complex_nested():
    x = Variable()
    numerator = Sum(
        Product(Sum(Polynomial(x, 2), Constant(1)), Sin(x)), Cos(Polynomial(x, 3))
    )
    ...
    def expected_derivative(x_val):
        num = (
            ((2 * x_val) * (1) + 0) * math.sin(x_val)
            + (x_val**2 + 1) * math.cos(x_val)
            + -math.sin(x_val**3) * 3 * x_val**2
        )
        den = x_val + math.tanh(x_val)
        der_num = (
            (2 * x_val) * math.sin(x_val)
            + (x_val**2 + 1) * math.cos(x_val)
            - 3 * x_val**2 * math.sin(x_val**3)
        )
