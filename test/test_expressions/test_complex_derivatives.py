...
def test_sum_product_division():
    x = Variable()
    expr = Division(Product(Sum(x, Constant(2)), Subtraction(x, Constant(3))), x)

    def expected_derivative(x_val):
        # d/dx [(x+2)(x-3)/x] = ((2x -1)*x - (x+2)(x-3))/x^2
        numerator = ((x_val + 2) * 1 + 1 * (x_val - 3)) * x_val - (x_val + 2) * (
            x_val - 3
        )
        denominator = x_val**2
...
def test_power_variable_exponent():
    ...
    expr = Power(x, x)  # x^x
    evaluate_derivative(
        expr, lambda x_val: x_val**x_val * (1 + math.log(x_val)), VALUES
    )
...
def test_log_product():
    ...
    evaluate_derivative(
        expr,
        lambda x_val: 1 / (x_val * (x_val + 1)) * ((1 * (x_val + 1) + x_val * 1)),
        VALUES,
    )
...
def test_trig_hyperbolic():
    ...
    evaluate_derivative(
        expr,
        lambda x_val: -math.sin(math.sinh(x_val)) * math.cosh(x_val)
        + 1 / (math.cosh(x_val) ** 2),
        VALUES,
    )
...
def test_negation_division():
    ...
    evaluate_derivative(
        expr, lambda x_val: -((1 * (x_val + 1) - x_val * 1) / (x_val + 1) ** 2), VALUES
    )
